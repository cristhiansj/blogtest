      ...
      <div class="span8">
           <?php if (!empty($article['comments'])): ?>
                ...
                <?php foreach($article['comments'] as $comment):
                    echo $comment['name'].'<strong> dice... </strong>';?>
          ...
          <form method="post" action="comment.php">
              <div><label for="fName">Nombre</label>
                  <input type="text" name="fName" id="fName" required="required" />
              </div>
              <div><label for="fEmail">Correo electrónico</label>
                <input type="email" name="fEmail" id="fEmail" required="required" placeholder="name@example.com" />
              </div>

              <div><label for="fQuestion">Pregunta/Comentario</label>
